...
function flowContract(graph: SemanticGraph, flowName: string): { intent: string; initialEvent: string; terminalEvent: string } {
  ...
  const initial = graph.edges
    .filter(edge => edge.type === 'FLOW_EMITS_EVENT' && edge.from === flowId)
    .sort((a, b) => order(a) - order(b))[0];
  const terminal = graph.edges
    .filter(edge => edge.type === 'FLOW_EXPECTS_EVENT' && edge.from === flowId)
...
export class FlowRuntime {
  ...
  async execute(flowName: string, initialPayload: unknown): Promise<ExecutionReport> {
    ...
    while (queue.length) {
      if (++transitions > 64) {
        return { status: 'Error', intent: contract.intent, last_event: lastEvent, payload: { message: 'Event choreography exceeded 64 transitions; possible cycle' } };
      }
      ...
      if (message.event === contract.terminalEvent) {
        terminalSeen = true;
        terminalPayload = message.payload;
        continue;
      }
      ...
      if (dispatches.length === 0) {
        return {
          ...
          payload: { message: `No Agent listens to ${message.event}` },
        };
      }
      ...
      for (const dispatch of dispatches) {
        ...
        if (!owner) {
          return { status: 'Error', intent: contract.intent, last_event: lastEvent, payload: { message: `Action ${action} has no owner` } };
        }
        const agent = label(owner.to);
        const listens = this.graph.edges.some(edge => edge.type === 'LISTENS' && edge.from === owner.to && edge.to === eventId);
        if (!listens) continue;

        const result = await this.agents.execute(agent, action, { state: this.state, payload: message.payload });
        if (result.status === 'Error') {
          this.events.emit(result.event, result.payload);
          return { status: 'Error', intent: contract.intent, last_event: result.event, payload: result.payload };
        }
    ...
    if (!terminalSeen) {
      return { status: 'Error', intent: contract.intent, last_event: lastEvent, payload: { message: `Flow ended without terminal Event ${contract.terminalEvent}` } };
    }

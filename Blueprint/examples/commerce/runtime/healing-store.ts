...
abstract class HealingStoreBase implements HealingCaseStore {
  ...
  protected hydrate(snapshot?: Partial<HealingStoreSnapshot>): void {
    for (const value of snapshot?.cases ?? []) this.cases.set(value.id, value);
  ...
  listPending(): HealingCase[] {
    const now=Date.now();
    let changed=false;
    for(const value of this.cases.values()) if(value.status==='pending-human'&&Date.parse(value.expires_at)<=now){value.status='expired';changed=true;}
    if(changed) this.changed();
    return [...this.cases.values()].filter(value=>value.status==='pending-human');
  }
  get(caseId: string): HealingCase | undefined { return this.cases.get(caseId); }

  escalate(input: Omit<HealingCase,'id'|'resume_token'|'created_at'|'expires_at'|'status'|'attempts'|'original_payload_hash'>, ttlMs: number): HealingCase {
    const existing=this.listPending().find(value=>value.correlation_id===input.correlation_id&&value.action===input.action);
    ...
    this.audits.push({at:new Date().toISOString(),kind:'human-escalation',case_id:value.id,agent:value.agent,action:value.action,detail:value.error_message});
    this.changed();
    return value;
  }
  ...
  consumeResume(caseId: string, token: string): HealingCase {
    ...
    this.audits.push({at:new Date().toISOString(),kind:'resume',case_id:value.id,agent:value.agent,action:value.action,detail:'Human context accepted for single resume.'});
    this.changed();
    return value;
  }
}
...
export class JsonFileHealingStore extends HealingStoreBase {
  constructor(readonly filePath:string){
    ...
    if(existsSync(filePath)){
      ...
      if(raw) this.hydrate(JSON.parse(raw) as HealingStoreSnapshot);
    }
  }
  protected override changed(): void {
    ...
    writeFileSync(this.filePath,readFileSync(temporary));
  }
}

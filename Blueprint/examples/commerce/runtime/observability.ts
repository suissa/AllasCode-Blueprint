...
export class ObservabilityRuntime {
 readonly traces:TraceRecord[]=[]; readonly audits:AuditRecord[]=[]; private readonly counters=new Map<string,number>();
 context(input:Partial<TraceContext>={}):TraceContext { const ctx:TraceContext={trace_id:input.trace_id??randomUUID(),correlation_id:input.correlation_id??randomUUID()}; if(input.causation_id!==undefined) ctx.causation_id=input.causation_id; if(input.operator_id!==undefined) ctx.operator_id=input.operator_id; return ctx; }
 trace(ctx:TraceContext,kind:TraceRecord['kind'],name:string,status:TraceRecord['status'],attributes:Record<string,unknown>={}):TraceRecord { const r:TraceRecord={id:randomUUID(),at:new Date().toISOString(),...ctx,kind,name,status,attributes:redact(attributes) as Record<string,unknown>}; this.traces.push(r); this.increment(`trace.${kind}.${status}`); return r; }
 audit(ctx:TraceContext,input:{operation:string;resource:string;operator_id?:string;outcome:'Ok'|'Error';attributes?:Record<string,unknown>}):AuditRecord { const operator_id=input.operator_id??ctx.operator_id; if(!operator_id) throw new Error('AuditOperatorRequired'); const r:AuditRecord={id:randomUUID(),at:new Date().toISOString(),...ctx,operator_id,operation:input.operation,resource:input.resource,outcome:input.outcome,attributes:redact(input.attributes??{}) as Record<string,unknown>}; this.audits.push(r); this.increment(`audit.${input.outcome}`); return r; }

...
export function diagnoseHealing(graph: SemanticGraph, context: HealingContext): HealingDecision {
  ...
  if (retry) {
    ...
    if (context.attempt + 1 < maxAttempts) {
      return { kind:'Retry', reason:`Graph strategy ${retry.label} matched the failure and preserves the same semantic request.`, strategy:retry.label, max_attempts:maxAttempts, timeout_ms:numberMeta(retry,'timeout_ms',1000), backoff_ms:numberMeta(retry,'backoff_ms',0) };
    }
  }
  ...
  if (human) return { kind:'Human', reason:`Graph strategy ${human.label} requires information that cannot be derived reversibly.`, strategy:human.label, required_context:stringArrayMeta(human,'required_context'), resume_ttl_ms:numberMeta(human,'resume_ttl_ms',86_400_000) };
...
async function executeBounded(execute: () => Promise<ActionResult>, timeoutMs: number, errorEvent: string): Promise<ActionResult> {
  ...
  return Promise.race([execute(),new Promise<ActionResult>(resolve=>setTimeout(()=>resolve({status:'Error',event:errorEvent,payload:{message:`timeout after ${timeoutMs}ms during healing retry`}}),timeoutMs))]);
}
...
export async function executeWithSemanticHealing(
  ...
  while(decision.kind==='Retry'&&attempts<decision.max_attempts){
    ...
    await backoff(decision.backoff_ms); attempts+=1;
  ...
  if(decision.kind==='Alternative'&&executeAlternative){
    ...
    activeAgent=decision.agent;activeAction=decision.action;attempts+=1;
    ...
    result=alternative; decision=diagnoseHealing(graph,{agent:activeAgent,action:activeAction,error:alternative,attempt:0});
  }
  ...
  if(decision.kind==='Human'&&store){
    ...
    const healingCase=store.escalate({intent:executionContext.intent??'unknown-intent',agent:activeAgent,action:activeAction,original_event:executionContext.original_event??'unknown-event',original_payload:executionContext.original_payload,error_event:result.event,error_message:result.payload.message,correlation_id:correlation},decision.resume_ttl_ms);
...
export async function resumeHumanHealing(store: HealingCaseStore,caseId:string,resumeToken:string,executeOriginal:(healingCase:HealingCase,humanPayload:unknown)=>Promise<ActionResult>,humanPayload:unknown):Promise<ActionResult>{

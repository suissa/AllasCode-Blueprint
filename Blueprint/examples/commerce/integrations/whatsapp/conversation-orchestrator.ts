...
export type ConversationState={conversation_id:string;peer:string;mode?:ConversationMode;pending_intent?:string;context:Record<string,unknown>;updated_at:string;expires_at:string;awaiting_context?:boolean};
...
export class WhatsAppConversationOrchestrator{
 ...
 async handle(message:InboundConversationMessage):Promise<ConversationReply>{
  ...
  if(missing){state.awaiting_context=true;await this.store.put(state);return{kind:'clarification',text:missing,correlation_id}}
  state.awaiting_context=false;await this.store.put(state);
  ...
  if(result.outcome==='Error'){
   if(result.error?.code==='MISSING_CONTEXT'||result.error?.code==='HEALING_REQUIRED'){state.awaiting_context=true;await this.store.put(state);return{kind:'clarification',text:result.error.message,correlation_id,outcome:result}}
 ...
 private missingContext(state:ConversationState,m:InboundConversationMessage):string|undefined{
  if(state.awaiting_context)return undefined;
  ...
  if(state.mode==='sale-resolution')return'Informe quais produtos pertencem a esta venda, por texto ou áudio.';
 }
}

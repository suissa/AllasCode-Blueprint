...
export class DeploymentController{
 private current?:ReleaseRecord; private previous?:ReleaseRecord;
 ...
 deploy(version:string,migrationIds:string[],checks:{health:boolean;readiness:boolean}):ReleaseRecord{
  ...
  this.previous=this.current; this.current={version,applied_migrations:this.migrations.snapshot(),ready:true}; return this.current;
 }
 rollback():ReleaseRecord{if(!this.previous)throw new Error('NoRollbackTarget');const failed=this.current;this.current=this.previous;this.previous=failed;return this.current;}

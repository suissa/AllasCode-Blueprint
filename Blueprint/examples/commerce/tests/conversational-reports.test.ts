...
test('demo is always dark glassmorphism and demonstrates animated 2D plus WebGL 3D families',async()=>{const root=new URL('../reports/demo/',import.meta.url);const [html,css,js]=await Promise.all([readFile(new URL('index.html',root),'utf8'),readFile(new URL('styles.css',root),'utf8'),readFile(new URL('demo.js',root),'utf8')]);assert.match(html,/echarts-gl@2/);assert.match(html,/class="chat glass"/);assert.match(css,/--bg:#030407/);assert.match(css,/backdrop-filter:blur/);for(const kind of ['line','bar','pie','radar','gauge','heatmap','treemap','sunburst','sankey','graph','bar3d','scatter3d','line3d','surface3d'])assert.ok(js.includes(`'${kind}'`),kind);assert.match(js,/getDataURL\(\{type:'png'/);assert.match(js,/crypto\.getRandomValues/);});

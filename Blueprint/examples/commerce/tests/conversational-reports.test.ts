...
test('chat reveals projection widgets progressively in declared order',()=>{const session=new ConversationalReportSession(report);const first=session.revealNext();if(first.outcome==='Error')assert.fail(first.error);assert.equal(first.value[0]!.widget_id,'w1');const next=session.revealNext(2);if(next.outcome==='Error')assert.fail(next.error);assert.deepEqual(session.visible().map(x=>x.widget_id),['w1','w2','w3']);});

test('share link token is opaque, expiring and owner-revocable',()=>{let now=Date.parse('2026-09-01T12:00:00Z');const shares=new ReportShareService('https://reports.example',()=>new Date(now));const made=shares.create(report,1000);if(made.outcome==='Error')assert.fail(made.error);assert.ok(made.value.share.token.length>=32);assert.equal(made.value.share.token.includes(report.report_id),false);assert.match(made.value.url,/\/reports\/share\//);assert.equal(shares.revoke(made.value.share.token,'other').outcome,'Error');assert.equal(shares.resolve(made.value.share.token).outcome,'Ok');now+=1001;assert.equal(shares.resolve(made.value.share.token).outcome,'Error');});

test('WhatsApp share package contains PNG preview and same unique report link',async()=>{const shares=new ReportShareService('https://reports.example');const preview:ReportPreviewRendererPort={renderPng:async(_r,url)=>({outcome:'Ok',value:{image_url:`https://cdn.example/preview.png?for=${encodeURIComponent(url)}`}})};const result=await shares.whatsApp(report,preview);if(result.outcome==='Error')assert.fail(result.error);assert.equal(result.value.mime_type,'image/png');assert.match(result.value.preview_image_url,/preview\.png/);assert.ok(result.value.caption.includes(result.value.share_url));});

test('all report widgets are projection-backed',()=>{assert.deepEqual(assertProjectionBacked(report),{outcome:'Ok',value:true});const invalid:ReportDocument={...report,widgets:[{...report.widgets[0]!,projection:''}]};assert.equal(assertProjectionBacked(invalid).outcome,'Error');});

...
const globalImpact = impact.changed_files.some(file => /\/governance\/|\/graph\/|\/config\.yml$|\/package\.json$|\/runtime\/semantic-(graph|governor|impact|ci-policy)\.ts$|\/scripts\/(semantic-impact|semantic-ci-policy|selector-confidence|run-selective-semantic-tests)\.ts$|\.github\/workflows\/commerce-example\.yml$/.test(file.replaceAll('\\','/')));

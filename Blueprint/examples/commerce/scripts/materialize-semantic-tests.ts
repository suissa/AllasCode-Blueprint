...
const ALL = ['unit','bdd','load','stress','chaos','synk','security','integration','e2e','benchmark'] as const;
...
const components: Record<string,string[]> = {
  ...
  stress:['summary-card','line','gauge'], chaos:['summary-card','timeline','status'], synk:['summary-card','table','status'], security:['summary-card','table','donut'],
...
const intents: Record<string,string> = {
  ...
  chaos:'Inject declared failures when a meaningful failure surface exists; otherwise record not-applicable with an explicit reason.',
...
const metricIds: Record<string,string[]> = {
  ...
  chaos:['faults_injected','invariants_preserved'], synk:['dependencies_scanned','vulnerabilities_total','high_findings'], security:['checks_total','findings_total','critical_findings'],
...
async function walk(path:string):Promise<string[]> { const out:string[]=[]; for(const e of await readdir(path,{withFileTypes:true})){ if(['node_modules','.git','tests','generated','.facop'].includes(e.name)) continue; const p=join(path,e.name); if(e.isDirectory()) out.push(...await walk(p)); else if(e.name==='manifest.yml') out.push(p); } return out; }

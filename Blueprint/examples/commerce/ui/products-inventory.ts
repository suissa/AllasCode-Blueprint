...
export class ProductsInventoryUi{
  ...
  renderList(state:{kind:'loading'}|{kind:'empty'}|{kind:'error';message:string}|{kind:'ready';items:ProductRow[];total:number;filters?:ProductListFilters}){
    ...
    const filterFields=Field({id:'products-search',label:'Buscar',value:f.search??'',hint:'Nome, SKU ou código'})+Field({id:'products-category',label:'Categoria',value:f.category??''});
    const filters=Filters({label:'Filtros de produtos',content:filterFields});
  ...
  renderEditor(input:{mode:'create'|'edit';draft:ProductDraft;result?:SemanticEnvelope}){
    ...
    const d=input.draft;
    return `<form class="product-editor" data-preserve-input="true" aria-label="${input.mode==='create'?'Criar':'Editar'} produto">${error}${Field({id:'product-name',label:'Nome',value:d.name,required:true})}${Field({id:'product-sku',label:'SKU',value:d.sku??''})}${Field({id:'product-category',label:'Categoria',value:d.category??''})}${Field({id:'product-unit',label:'Unidade',value:d.unit??''})}${Field({id:'product-minimum-stock',label:'Estoque mínimo',type:'number',value:String(d.minimum_stock??0)})}<label><input type="checkbox" name="active" ${d.active===false?'':'checked'}> Produto ativo</label><button type="submit">Salvar</button></form>`;
  }
